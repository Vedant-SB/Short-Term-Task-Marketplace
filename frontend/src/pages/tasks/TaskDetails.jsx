...
function TaskDetails() {
  ...
  return (
    <div className="relative min-h-[calc(100vh-4rem)] overflow-hidden bg-canvas">
      ...
      <div className="relative mx-auto max-w-5xl px-5 pt-8 pb-16 md:px-8 md:pt-12">
        {/* ═══════════════════════════════════════════════
            1. BACK LINK
            ═══════════════════════════════════════════════ */}
        <motion.div {...stagger(0)}>
          <Link
            to="/tasks"
            className="group mb-6 inline-flex items-center gap-1.5 text-sm font-medium text-violet-600 transition-colors hover:text-violet-800"
          >
        ...
        <motion.div
          {...stagger(0.05)}
          className="group relative mb-6 overflow-hidden rounded-2xl border border-zinc-200/80 bg-white shadow-[0_10px_28px_rgba(15,23,42,0.07)]"
        >
          {/* Urgency wash on far right */}
          <div
            className={`pointer-events-none absolute right-0 top-0 bottom-0 w-[10%] min-w-[70px] bg-gradient-to-l ${uStyle.wash} opacity-75`}
          />
          <div
          ...
          <div className="relative flex flex-col gap-6 p-6 md:flex-row md:p-8">
            ...
            <div className="min-w-0 flex-1">
              ...
              {task.skillsRequired && task.skillsRequired.length > 0 && (
                <div className="mt-4 flex flex-wrap gap-2">
                  {task.skillsRequired.slice(0, 5).map((s) => (
                    <SkillChip key={s}>{s}</SkillChip>
                  ))}
                  {task.skillsRequired.length > 5 && (
                    <span className="inline-flex items-center rounded-full border border-violet-200/70 bg-violet-50/85 px-3 py-1.5 text-xs font-semibold text-violet-700">
                      +{task.skillsRequired.length - 5}
                    </span>
                  )}
                </div>
            ...
            {/* Vertical divider */}
            <div className="hidden w-px bg-zinc-200/80 md:block" />

            {/* Right side — budget, status, deadline */}
            <div className="flex shrink-0 flex-col items-start gap-3 md:min-w-[200px] md:items-end md:text-right">
              <p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-slate-400">
                Budget
              </p>
              ...
              {/* Status badge */}
              <span
                className={`inline-flex items-center gap-1.5 rounded-full border px-4 py-1.5 text-xs font-bold leading-none ${statusInfo.cls}`}
              >
                {statusInfo.label}
              </span>
              ...
              {/* Days left badge */}
              <span
                className={`inline-flex items-center gap-2 rounded-full border px-3.5 py-1.5 text-xs font-bold shadow-sm ${uStyle.bg} ${uStyle.text} ${uStyle.border}`}
              >
                <span
                  className={`h-2 w-2 rounded-full ${uStyle.dot}`}
                />
        ...
        <SectionCard delay={0.2} className="mb-6">
          ...
          <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
            ...
            <InfoItem
              icon={IndianRupee}
              label="Budget"
              value={`₹${task.budget?.toLocaleString("en-IN") ?? 0}`}
            />
          </div>
        </SectionCard>
        ...
        {/* ═══════════════════════════════════════════════
            ...
            ═══════════════════════════════════════════════ */}
        {task.skillsRequired && task.skillsRequired.length > 0 && (
          <SectionCard delay={0.25} className="mb-6">
        ...
        <SectionCard delay={0.3} className="mb-6">
          ...
          <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
            ...
            {/* Right — Rating card */}
            <div className="shrink-0 rounded-xl border border-zinc-200/80 bg-gradient-to-br from-violet-50/60 to-white p-5 text-center lg:min-w-[220px]">
                ...
                <>
                  ...
                  <p className="mt-2 text-xs text-slate-400">
                    ...
                    {companyReviewCount != null
                      ? `${companyReviewCount} verified review${
                          companyReviewCount !== 1 ? "s" : ""
                        }`
                      : "verified reviews"}
                  </p>
                </>
        ...
        {(eligibilityAndPreferences.length > 0 ||
          ...
          <SectionCard delay={0.35} className="mb-6">
        ...
        {/* ═══════════════════════════════════════════════
            ADDITIONAL INFO (dates, extensions — always)
            ═══════════════════════════════════════════════ */}
        <SectionCard delay={0.38} className="mb-6">
        ...
        {message && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            className="mb-6 rounded-xl border border-violet-200/60 bg-violet-50/70 px-5 py-3.5 text-sm font-medium text-violet-800"
          >
            {message}
          </motion.div>
        )}
        ...
        {/* ═══════════════════════════════════════════════
            8. BOTTOM CTA CARD
            ═══════════════════════════════════════════════ */}
        {user?.role === "individual" && task.status === "open" && (
          <motion.div
            {...stagger(0.45)}
            className="rounded-2xl border border-zinc-200/80 bg-white p-6 shadow-[0_8px_24px_rgba(15,23,42,0.06)] md:p-8"
            style={{
              backgroundImage:
                "linear-gradient(135deg, rgba(139,92,246,0.04), rgba(255,255,255,0.95))",
            }}
          >
            <div className="flex flex-col items-start justify-between gap-5 md:flex-row md:items-center">
              <div>
                ...
                <p className="mt-1.5 text-sm text-slate-500">
                  Showcase your skills and get selected for exciting
                  opportunities.
                </p>
              </div>
              ...
              <div className="flex shrink-0 items-center gap-3">
                ...
                {!hasApplied && (
                  <button
                    ...
                    disabled={
                      applying ||
                      new Date() > new Date(task.applicationDeadline)
                    }
                    className="inline-flex items-center gap-2 rounded-xl bg-primary px-6 py-3 text-sm font-semibold text-primary-foreground shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-elegant disabled:opacity-50 disabled:hover:translate-y-0"
                  >
                    <Send className="h-4 w-4" strokeWidth={1.8} />
                    {applying
                      ? "Applying..."
                ...
                {hasApplied && applicationStatus === "pending" && (
                  <div className="flex items-center gap-3">
                    <span className="inline-flex items-center gap-1.5 rounded-full border border-emerald-300 bg-emerald-50 px-4 py-2 text-sm font-bold text-emerald-700">
                      <CheckCircle2 className="h-4 w-4" />
                      Already Applied
                    </span>
                    <button
